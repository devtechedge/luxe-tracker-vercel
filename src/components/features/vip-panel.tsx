  ...
  return (
    <Card>
      <CardHeader>
        <div>
          <CardTitle>
            <Crown className="size-4 text-[var(--color-accent)]" />
        ...
        >
          {sim.qualifiedTier ? (
            <div>
              ...
              <div className="mt-1 flex items-baseline gap-2">
                <span className="text-3xl font-bold text-[var(--color-ink)]">{sim.qualifiedTier.tierName}</span>
                <span className="text-sm text-[var(--color-ink-muted)]">at {brand.name}</span>
              </div>
          ...
          {sim.nextTier && (
            <div className="mt-3 text-[11px] text-[var(--color-ink-muted)]">
              Spend <span className="font-mono text-[var(--color-accent)]">{fmtEUR(sim.nextTier.gapEUR, 0)}</span> more to unlock{' '}
              <span className="font-semibold text-[var(--color-ink)]">{sim.nextTier.tierName}</span>.
            </div>
          )}
        </div>
        ...
        <div className="space-y-2">
          {sim.allTiers.map((tier, i) => {
            ...
            return (
              ...
              >
                <div className="flex size-10 items-center justify-center rounded-md bg-[var(--color-surface-2)] text-lg">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-[var(--color-ink)]">{tier.tierName}</span>
                ...
                <div className="text-right text-[11px]">
                  <div className="text-[var(--color-ink-muted)]">Priority</div>
                  <div className="font-mono text-[var(--color-ink)]">{tier.allocationPriority}/5</div>
                </div>
              </div>
            )
          })}
...
function Benefit({ icon, label, value }: { icon: ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] p-2">
      <div className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-[var(--color-ink-muted)]">
        {icon}
        {label}
      </div>
      <div className="mt-0.5 font-mono text-sm font-bold text-[var(--color-ink)]">{value}</div>
    </div>
  )
}
